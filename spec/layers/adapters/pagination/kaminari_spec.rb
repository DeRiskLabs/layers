...
RSpec.describe Layers::Adapters::Pagination::Kaminari do
  ...
  describe '.per' do
    before do
      allow(relation).to receive(:per).and_return(refined_relation)
    end
    ...
    it 'sets the page size on the relation' do
      expect(relation).to have_received(:per).with(25)
    end
